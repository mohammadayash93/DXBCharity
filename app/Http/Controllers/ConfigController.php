...
class ConfigController extends Controller
{
    ...
    public function update(ConfigRequest $request, $id)
    {
    	...
    	if($request->file('caminho_img_login')){
            ...
            $path = $file->move("img/config", "login_logo.{$ext}");
    		Config::where('id', 1)->update(['caminho_img_login' => "img/config/login_logo.{$ext}"]);
    	}
        ...
        if($request->file('app_logo')){
    		...
    		Config::where('id', 1)->update(['app_logo' => "img/config/logo.{$ext}"]);
    	}
